...
# 機能: ログイン、信用 売り 返済注文（返済建玉個別指定以外）、ログアウト を行ないます。
#
...
#
# 注意: このサンプルは、建日種類 1:個別指定のものです。
#       2:建日順、3:単価益順、4:単価損順 のサンプルは、信用返済買いを参照してください。
#
# 要求電文
# {
...
# 機能: 信用(制度信用6ヶ月) 売り 返済 注文（返済建玉個別指定以外）
# 返値： 辞書型データ（APIからのjson形式返信データをshift-jisのstring型に変換し、更に辞書型に変換）
# 引数1: p_no
# 引数2: 銘柄コード
# 引数3: 市場（現在、東証'00'のみ）
