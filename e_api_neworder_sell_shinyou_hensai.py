...
# 機能: ログイン、信用返済売り注文（返済建玉個別指定以外）、ログアウト を行ないます。
#
...
# 機能: 信用返済(制度信用6ヶ月) 売り注文（返済建玉個別指定以外）
# 返値： 辞書型データ（APIからのjson形式返信データをshift-jisのstring型に変換し、更に辞書型に変換）
# 引数1: p_no
# 引数2: 銘柄コード
# 引数3: 市場（現在、東証'00'のみ）
...
def func_neworder_sell_sinyou_close(int_p_no,
                                    str_sIssueCode,
                                    str_sSizyouC,
                                    str_sCondition,
                                    str_sOrderPrice,
...
# 銘柄コード: my_code （実際の銘柄コードを入れてください。）
# 市場: my_shijyou （00:東証   現在(2021/07/01)、東証のみ可能。）
# 執行条件: my_shikkou   （0:指定なし、2:寄付、4:引け、6:不成。指し値は、0:指定なし。）
...
# コマンド用パラメーター -------------------    
# 信用 返済 売り 注文パラメーターセット（返済建玉個別指定以外）
...
if bool_login :
    ...
    print()
    print('-- 信用 返済売り注文（個別指定以外）  -------------------------------------------------------------')
    int_p_no = int_p_no + 1
    # 信用 返済 売り注文    引数：p_no、銘柄コード、市場（現在、東証'00'のみ）、執行条件、価格、株数、口座属性クラス
    dic_return = func_neworder_sell_sinyou_close(int_p_no,
                                                my_sIssueCode,
                                                my_sSizyouC,
                                                my_sCondition,
                                                my_sOrderPrice,
